...
def read():
    variables = []
    variablesNum = []
    Archivo = input('Archivo que deseas procesar (solo el nombre sin poner la extensión .txt): ')
    with open(f'{Archivo}.txt') as file:
        for line in file:
    ...
    PorcentajeTest = float(input('\nPorcentaje de registros test: '))/100
    ...
    PesosIniciales = float(input('\nQué peso quieres usar como inicial: '))
    alpha = float(input('\nQué alpha quieres usar: '))
    Epocas = int(input('\nCuantas epocas quieres: '))
    accuracyUsuario = float(input('\ncuanta accuracy te gustaría tener en consideracion: '))/100
    #PesosIniciales = 0.1
    #alpha = 0.000001
    #Epocas = 100
...
def calculaO(pesos,xVariablesTrain,variablesNum,alpha,xVariablesTest,tTest,Epocas,accuracyUsuario):
    ...
    plt.title("O Final Calculada vs T del Input")
    plt.plot(oFinal)
    plt.plot(tTest)
    plt.legend(["O Final", "T del Input"], loc ="upper right")
    plt.show()
